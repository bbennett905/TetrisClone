...
void rotate(tetromino_t* block)
{
	...
	{
		//Least X, greatest Y
		if (copy._blocks[i]._pos.x + copy._pos.x < top_left.x) 
			top_left.x = copy._blocks[i]._pos.x + copy._pos.x;
		if (copy._blocks[i]._pos.y + copy._pos.y > top_left.y) 
			top_left.y = copy._blocks[i]._pos.y + copy._pos.y;
	}
	vec2d8_t diff = sub(old_pos, top_left);
	for (int i = 0; i < 4; i++)
	{
		copy._blocks[i]._pos = add(copy._blocks[i]._pos, diff);
	}
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (copy._blocks[i]._pos.x + copy._pos.x < 0 || 
			copy._blocks[i]._pos.x + copy._pos.x >= FIELD_WIDTH) return;
		if (copy._blocks[i]._pos.y + copy._pos.y < 0 || 
			copy._blocks[i]._pos.y + copy._pos.y >= FIELD_HEIGHT) return;
	}
